...
{ ------------------------------------------------------------------
  ...
  VARIANTE UNA VENTANA: captura y matriz resultante dentro de la ventana.
  ------------------------------------------------------------------ }
...
var i, j: integer;
begin
  Abre_ventana('CAPTURA MATRIZ ' + Nombre);
  for i := 1 to F do
    for j := 1 to C do
...
var A, B, R: TMatriz; f, c, i, j: integer;
begin
  Abre_ventana('RESTA DE MATRICES');
  f := Lee_entero_rango('Filas (1 a 10): ', 1, MAXM);
  c := Lee_entero_rango('Columnas (1 a 10): ', 1, MAXM);
  Captura_matriz(A, f, c, 'A');
  Captura_matriz(B, f, c, 'B');
...
begin
  Abre_ventana('MATRIZ TRANSPUESTA');
  f := Lee_entero_rango('Filas (1 a 10): ', 1, MAXM);
  c := Lee_entero_rango('Columnas (1 a 10): ', 1, MAXM);
  Captura_matriz(A, f, c, 'A');
...
begin
  Abre_ventana('MATRIZ INVERSA 2x2');
