...
{ ------------------------------------------------------------------
  ...
  Datos en arreglos [1..75], lectura con Val, resultados en panel grafico.
  ------------------------------------------------------------------ }
...
begin
  WriteLn; WriteLn('=== MEDIA ARITMETICA ===');
  Captura_datos(D, N, 1, MAXP);
  media := Suma_de(D, N) / N;
...
begin
  WriteLn; WriteLn('=== ORDENAR DE MENOR A MAYOR ===');
  Captura_datos(D, N, 1, MAXP);
  Ordena(D, N, True);
...
var D: TArregloReal; N: integer;
begin
  WriteLn; WriteLn('=== ORDENAR DE MAYOR A MENOR ===');
  Captura_datos(D, N, 1, MAXP);
  Ordena(D, N, False);
...
begin
  WriteLn; WriteLn('=== REGRESION LINEAL SIMPLE  Y = a + b X ===');
...
begin
  WriteLn; WriteLn('=== VARIANZA Y DESVIACION ESTANDAR ===');
