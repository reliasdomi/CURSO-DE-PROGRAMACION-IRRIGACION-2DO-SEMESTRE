...
begin
  ...
  FBanner.Color := clNavy;
  FBanner.ParentBackground := False;
  FBanner.Font.Color := clWhite;
  FBanner.Font.Style := [fsBold];
  FBanner.Font.Size := 11;
  FBanner.Alignment := taCenter;
  FBanner.Caption := 'UNIVERSIDAD AUTONOMA CHAPINGO - DEPARTAMENTO DE IRRIGACION'
    + sLineBreak + 'PROGRAMACION - GRUPO 4A';

  FContent := TPanel.Create(Self);
  FContent.Parent := Self;
  FContent.Align := alClient;
  FContent.BevelOuter := bvNone;
  FContent.Color := clWindow;
